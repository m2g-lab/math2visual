...
def extract_visual_language(text):
    ...
    if last_index != -1:
        return text[
            last_index:
        ].strip()  # Extract and return everything after the last occurrence
    else:
...
def render_svgs_from_data(output_file, resources_path, data):
    ...
    def handle_comparison(
        ...
        for i in range(2):
            ...
            try:
                created, w, h = handle_all_except_comparison(
                    operations_i,
                    entities_i,
                    svg_root,
                    resources_path,
    ...
    def draw_balance_scale(svg_root, entity_boxes):
        ...
        print(
            "right_x, right_y, right_w, right_h: ", right_x, right_y, right_w, right_h
        )
        ...
        right_vertical_plate_stick_x = (right_x + right_w / 2.0) - (
            right_vertical_plate_stick_width / 2.0
        )
    ...
    def handle_all_except_comparison(
        operations,
        entities,
        svg_root,
        resources_path,
        ...
        if normal_entities:
            largest_normal_q = max(
                e["item"].get("entity_quantity", 0) for e in normal_entities
            )
        else:
            largest_normal_q = 1
        ...
        normal_box_height = (
            max_rows * (ITEM_SIZE + ITEM_PADDING + unit_trans_padding) + BOX_PADDING
        )
        ...
        for i, entity in enumerate(entities):
            ...
            if operations and i < len(operations):
                ...
                print(
                    'operator entities[0]["planned_height"]: ',
                    entities[0]["planned_height"],
                )
        ...
        eq_y = (
            position_box_y + (entities[0]["planned_height"] / 2) - (OPERATOR_SIZE / 2)
        )
        ...
        def embed_svg(file_path, x, y, width, height):
            ...
            if not os.path.exists(file_path):
                ...
                if dir_path in _svg_directory_cache:
                    ...
                    candidate_files = [
                        f for f in os.listdir(dir_path) if f.lower().endswith(".svg")
                    ]
                ...
                def try_candidate(name):
                    for candidate in candidate_paths:
                        candidate_base = os.path.splitext(os.path.basename(candidate))[
                            0
                        ]
                ...
                if not found_path:
                    ...
                    close_matches = difflib.get_close_matches(
                        base_name, candidate_bases, n=1, cutoff=0.6
                    )
                    if close_matches:
                ...
                if found_path:
                    ...
                    error_message = (
                        f"SVG file not found using alternative search: {file_path}"
                    )
        ...
        def embed_top_figures_and_text(
            parent,
            box_x,
            box_y,
            box_width,
            ...
            items = []
            show_something = (
                container_name or container_type or attr_name or attr_entity_type
            )
            ...
            if not show_something:
                ...
                if container_type:
                    ...
                    if figure_path and os.path.exists(figure_path):
                        ...
                        print(
                            f"SVG for container_type '{container_type}' does not exist. Ignoring container_type."
                        )
            ...
            for idx, (t, v) in enumerate(items):
                if t == "svg":
                    ...
                    width = (
                        len(v) * 7
                    )  # Approximate width per character at font-size 15px
        ...
        def draw_entity(e):
            ...
            if layout == "multiplier":
                ...
                print(
                    "2 update_max_dimensions: ", text_x + len(q_str) * 30, text_y + 50
                )
                return
            # Draw box
            etree.SubElement(
            ...
            try:
                if e.get("bracket") == "left":
                    ...
                    operator_y = position_box_y + (
                        entities[0]["planned_height"] * 1.2 / 2
                    )
                    text_element = etree.SubElement(
                        svg_root,
                        "text",
            ...
            if layout == "large":
                ...
                if unittrans_unit and unittrans_value is not None:
                    text_element = etree.SubElement(
                        ...
                        x=str(text_x),
                        y=str(text_y),
                        style="font-size: 100px; fill: white; font-weight: bold; stroke: black; stroke-width: 2px;",
                        dominant_baseline="middle",
                    )
                else:
                    text_element = etree.SubElement(
                        ...
                        x=str(text_x),
                        y=str(text_y),
                        style="font-size: 45px; fill: white; font-weight: bold; stroke: black; stroke-width: 2px;",
                        dominant_baseline="middle",
                    )
                text_element.text = q_str
                update_max_dimensions(start_x_line + tw, center_y_line + 40)
                print(
                    "4 update_max_dimensions: ", start_x_line + tw, center_y_line + 40
                )
                ...
                if unittrans_unit and unittrans_value is not None:
                    ...
                    circle_center_y = (
                        svg_y - circle_radius
                    )  # Above the top-right corner of the item
        ...
        if operations:
            ...
            for operator in operations:
                ...
                mapped_operator_entity_type = operator_svg_mapping.get(
                    operator_entity_type, operator_entity_type
                )  # Fallback to itself if not in mapping

                # Determine the SVG file path
                operator_svg_path = os.path.join(
                    resources_path, f"{mapped_operator_entity_type}.svg"
                )
                ...
                if not os.path.exists(operator_svg_path):
                    ...
                    operator_svg_path = os.path.join(
                        resources_path, f"{fallback_entity_type}.svg"
                    )
        ...
        if not os.path.exists(equals_svg_path):
            equals_svg_path = os.path.join(
                resources_path, "equals_default.svg"
        ...
        if operations and operations[-1]["entity_type"] == "surplus":
            ...
            if not os.path.exists(question_mark_svg_path):
                question_mark_svg_path = os.path.join(
                    resources_path, "question_default.svg"
                )  # Fallback if necessary
            svg_root.append(
                embed_svg(
                    question_mark_svg_path, x=qmark_x, y=qmark_y, width=60, height=60
                )
            )
            ...
            text_x = (
                qmark_x + 70
            )  # Adjust spacing to place text after the first question mark
        ...
        return (
            True,
            str(float(svg_root.attrib["width"]) - start_x),
            svg_root.attrib["height"],
        )
    ...
    if data.get("operation") == "comparison":
        ...
        compare1_entities, compare1_result_entities = update_container_types_optimized(
        ...
        if compare1_result_entities and compare1_entities:
            ...
            if (
                any(
                    e.get("container_name") == last_container for e in compare1_entities
                )
                and last_container
            ):
                compare1_result_entities[-1][
                    "container_name"
                ] = f"{last_container} (result)"

        # if find container_name of different entity are different but the container entity_type are the same, update the second entity's container entity_type to be original entity_type-2,the third to be original entity_type-3...
        compare2_entities, compare2_result_entities = update_container_types_optimized(
        ...
        if compare2_result_entities and compare2_entities:
            ...
            if (
                any(
                    e.get("container_name") == last_container for e in compare2_entities
                )
                and last_container
            ):
                compare2_result_entities[-1][
                    "container_name"
                ] = f"{last_container} (result)"
        ...
        try:
            created, svg_width, svg_height = handle_comparison(
                compare1_operations,
                compare1_entities,
                compare1_result_entities,
                compare2_operations,
        ...
        # if find container_name of different entity are different but the container entity_type are the same, update the second entity's container entity_type to be original entity_type-2,the third to be original entity_type-3...
        entities, result_entities = update_container_types_optimized(
        ...
        if result_entities and entities:
            ...
            if (
                any(e.get("container_name") == last_container for e in entities)
                and last_container
            ):
        ...
        try:
            created, svg_width, svg_height = handle_all_except_comparison(
                operations, entities, svg_root, resources_path, result_entities
            )
        except:
            created = False
...
if __name__ == "__main__":
    ...
    visual_language = [
        "comparison(container1[entity_name: apple, entity_type: apple, entity_quantity: 5, container_name: Sarah, container_type: girl, attr_name: , attr_type: ], container2[entity_name: apple, entity_type: apple, entity_quantity: 15, container_name: Tom, container_type: boy, attr_name: , attr_type: ], result_container[entity_name: apple, entity_type: apple, entity_quantity: , container_name: Tom, container_type: more, attr_name: , attr_type: ])"
    ]
